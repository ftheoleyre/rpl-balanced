...
int init(call_t *c, void *params) {
	printf("[RPL_INIT] Start RPL ELT for journal.\n");	
...
int setnode(call_t *c, void *params) {
	...
	nodedata->energy_spent = 0.0;
	nodedata->stats_data_tx = 0;
	
	//for elt 
	nodedata->start_energy	= START_ENERGY; //Joule (= 880 mAh)
	nodedata->energy_residual = nodedata->start_energy;
	nodedata->my_traffic 	= 1.0; // equivalent of sending 1 pkt every 1000 seconds  (1 bit/s)
	nodedata->old_tx 	 	= 0.0; //for the moving average of the traffic
...
int bootstrap(call_t *c) {
	...
	//scheduler_add_callback(get_time(), c, verify_bottleneck, NULL);
...
int set_header (call_t *c, packet_t *packet, destination_t *dst) {
	...
	if(prefer_parent != NULL){
		header->next = prefer_parent->id;
		destination.id = prefer_parent->id;
	}
	else{
...
void forward(call_t *c, packet_t *packet){
	...
	//set the destination to be the preferred parent
	if(prefer_parent != NULL){
		header->next = prefer_parent->id;
		destination.id = prefer_parent->id;
	}
...
void tx (call_t *c, packet_t *packet) {
	...
	//printf("[RPL]Node %d sent packet to %d sequence %d wsize %d.\n", c->node, header->next, header->pkt_sequence, packet->size);
...
void rx(call_t *c, packet_t *packet) {
	...
	switch(header->type_code){
		...
		case RPL_CODE_DIO:
			...
			//printf("\t[RPL_RX_DIO] Node ID = %d at T = %llu from %d size = %d, has residual energy %f\n", c->node, get_time(), header->source, packet->size, dio->path_energy_residual);
		...
		case RPL_DATA_PACKET:
			...
			//printf("\t[RPL_RX_DATA] Node = %d received From  = %d sequence %d header->next = %d\n", c->node, header->source, header->pkt_sequence, header->next);
