...
//    printf("option %d\n", option);
    ...
    switch(option){
        ...
        case RADIO_IOCTL_GETENCONSUMMED:
            ;
           //printf("ptr set: %d time %llu\n", &(nodedata->energy), get_time());
           // printf("energy consumption is: %e duration: %llu time %llu\n", nodedata->energy.consumption, nodedata->energy.duration, get_time());

            //energy_t **out_en;
            *out = &(nodedata->energy);  
            //printf("RADIO Node %d energy pointer %d\n ", c->node, &nodedata->energy);  
            //printf("Radio Node %d energy tx %f\n ", c->node, nodedata->energy.consumption);          
...
void tx(call_t *c, packet_t *packet) {
    ...
    while (i--) {
        ...
        //if(packet->size == 127)
        //	printf("!!!!!!!!!![Radio_TX] Node %d transmitted the pkt on the medium size %d\n", c->node, packet->size );
    }
