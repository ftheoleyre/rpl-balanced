...
// 	cbrv3_packet_header		*header_cbr		= (cbrv3_packet_header *) (packet->data + sizeof(_802_15_4_header) + sizeof(_802_15_4_DATA_header));
	...
	if (nodedata->addr_short == ADDR_INVALID_16){
		...
		tools_debug_print(c, "\n\n[MAC_DATA] DATA message received from %d to %d via %d -> dropped: I am not yet associated\n\n\n", header_data->src_end2end, header_data->dst_end2end, header_data->src);
		stats_data_update_drop_reason(c, packet, DROP_NOT_ASSOCIATED);
		data_drop_packet(c, packet);
		printf("\t\t--------------------------------[MAC_DATA] node %d DATA -> dropped: I am not yet associated\n\n\n", c->node);
	...
	//printf("[MAC_Data_Route_Up]Node %d put packet in the up queue\n", c->node);
...
// header_data->dst = routing_ideal_get_next_hop(c, header_data->dst_end2end);
	...
	if (header_data->dst == ADDR_INVALID_16){
		...
		tools_debug_print(c, "\n\n[MAC_RX_DATA] DATA message received from %d to %d via %d -> dropped: no next hop\n\n\n", header_data->src_end2end, header_data->dst_end2end, header_data->src);
		data_drop_packet(c, packet);
		printf("\t\t--------------------------------[MAC_DATA] node %d DATA -> dropped: no next hop\n\n\n", c->node);
...
int data_route_final_dest(call_t *c, packet_t *packet_rcvd){
	...
	tools_debug_print(c, "[MAC_RX_DATA] DATA message finally received from ID = %d via %d", header_data->src_end2end, header_data->src);
	...
	*/
	...
	if (up->size > 1){
		printf("does not support several upper layers\n");
		exit(65);
	}					

	if (!tools_is_addr_multicast(header_data->dst)){
		total++;
		//printf("\t [MAC_Data_Route_Final_Dest] Node %d received pkt in unicast from %d, time %llu, total %d\n", c->node, header_data->src, get_time(), total);
	}
	else{
		//printf("\t\t\t\t\t[MAC_RX_DATA] Node %d DIO received from %d size %d\n", c->node, header_data->src, packet_rcvd->size);

		//OANA TODO
		//verify if the packet is from a father or from a son 
		//verify if the father is associated 
		
		//verify if the sender is in my parent list and if i'm associated to him
		parent_info	*pinfo_current = ctree_parent_associated_get_info(nodedata->parents, header_data->src);
    	if (pinfo_current == NULL){
			//printf("not associated to him anymore");
			//is not my parent, check if sender is my son  	
			//if() uita-te in functia de printare a informatiilor HasChild
			int found = 0;
			...
			while ((neigh_elem = (neigh_info*) das_traverse(nodedata->neigh_table)) != NULL)
				if(neigh_elem->addr_short == header_data->src && neigh_elem->is_child)
					found = 1;
		...
		//printf("\t\t\t\t\t[MAC_RX_DATA] not dealloc Node %d DIO received from %d size %d\n", c->node, header_data->src, packet_rcvd->size);
	}
...
void data_route(call_t *c, packet_t *packet_rcvd){
	...
	//MULTICAST
    else if (tools_is_addr_multicast(header_data->dst)){
        ...
        if (header_data->dst == appli_data->multicast_dest){
        ...
        //flooding packet
        else if ((header_data->dst == ADDR_MULTI_FLOOD_DISC) || (header_data->dst == ADDR_MULTI_FLOOD_CTREE))
            data_route_multicast(c, packet_rcvd);
        
        //we have received a packet generated by a neighbor
        else{
            //packet_dealloc(packet_rcvd);
            routing_error = data_route_final_dest(c, packet_rcvd);
            }
	...
	//UNICAST: find the correct queue (UP or DOWN)
       ...
       tools_debug_print(c, "[MAC_RX_DATA] Unicast received from %d to %d (via %d, next hop %d)\n", 
		...
		//tmp header_data->src = nodedata->addr_short;
		
		//upload direction -> we forward the packet to our parent
		//if (header_data->dst_end2end == ID_PAN_COORD)
		
		//seul cqs q considerer
		//tmp	routing_error = data_route_up(c, packet_rcvd);
		
		//in download direction, we search for the next hop
		//else	
		//	routing_error = data_route_down(c, packet_rcvd);
		
		if (header_data->src == c->node){
			//printf("[MAC_Data_Route]Node %d sent data in unicast, time %llu, send it up\n", c->node, get_time());
           	routing_error = data_route_up(c, packet_rcvd);
           	
			//oana: update information about nr of packets received from RPL 
			etx_elt_t *etx_elt = (etx_elt_t *) malloc(sizeof(etx_elt_t));								
			int found = 0;																																																						
			...
			while((etx_elt = (etx_elt_t*) das_traverse(nodedata->neigh_retransmit)) != NULL){
				if (etx_elt->neighbor == header_data->dst) {
					found = 1;
					etx_elt->packs_net++;
					//printf(">>>>>>>>>>>>>>>>>>>>>> 1 Node %d sent to node %d, %d packets\n", c->node, nre->neighbor, nre->pkt_rcv_from_rpl);
					
				}
			}
			if(found == 0){
				etx_elt = (etx_elt_t *) malloc(sizeof(etx_elt_t));								
				etx_elt->neighbor 		= header_data->dst;
				etx_elt->packs_net 		= 1;	
				etx_elt->packs_tx 		= 0;
				etx_elt->packs_tx_once	= 0;
				etx_elt->acks			= 0;
				etx_elt->drops			= 0;
				das_insert(nodedata->neigh_retransmit, etx_elt); 
				//printf(">>>>>>>>>>>>>>>>>>>>>> 0 Node %d sent to node %d, %d packets\n", c->node, nre->neighbor, nre->pkt_rcv_from_rpl);          	
			}
        }
		else{
			//printf("[MAC_Data_Route]Node %d received pkt in unicast from %d (%d), time %llu, send it to data_route_final_dest\n", c->node, header_data->src,header_data->src_end2end, get_time());
			routing_error = data_route_final_dest(c, packet_rcvd);
		}	
...
void data_drop_packet(call_t *c, packet_t *pkt_drop){
	nodedata_info *nodedata = get_node_private_data(c);
	//packet header
  	_802_15_4_DATA_header	*header_data	= (_802_15_4_DATA_header *) (pkt_drop->data + sizeof(_802_15_4_header));
	//printf("Node %d data_drop_packet for %d\n", c->node, header_data->dst);
					
	//for ETX ELT - count the no of pkt drops
	etx_elt_t *etx_elt; 								
	das_init_traverse(nodedata->neigh_retransmit);
	while((etx_elt = (etx_elt_t*) das_traverse(nodedata->neigh_retransmit)) != NULL){
		if (etx_elt->neighbor == header_data->dst) {
